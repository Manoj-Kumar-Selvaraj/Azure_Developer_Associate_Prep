...
| Log Type                               | Purpose                                         | Enabled By                          |
| -------------------------------------- | ----------------------------------------------- | ----------------------------------- |
| **Application Logging** (aka App Logs) | Logs generated by your **application code**     | App Service settings                |
| **Web Server Logging**                 | Logs **HTTP requests** to your app (IIS logs)   | File system or Azure Blob           |
| **Detailed Error Messages**            | Shows **detailed 500 errors**                   | App Service setting                 |
| **Failed Request Tracing (FREB)**      | Diagnose **failed requests**, especially on IIS | Useful for **slow or broken pages** |
| **Container Logs**                     | For containerized web apps – logs stdout/stderr | Docker logging                      |

---

| Feature              | Purpose                                                           |
| -------------------- | ----------------------------------------------------------------- |
| Application Logging  | Logs from your app code (e.g. Python `print`, Node `console.log`) |
| Web Server Logs      | IIS access logs (HTTP request/response)                           |
| FREB                 | Debugging failed requests (e.g., 500, slow requests)              |
| Kudu                 | Access file system, log stream, debug tools                       |
| App Insights         | End-to-end telemetry: metrics, exceptions, traces                 |
| `az webapp log tail` | Live log view in CLI                                              |
| File system logs     | Temporary (max 100 MB)                                            |
| Blob logs            | Persistent storage for logs                                       |
...
| Destination                  | Notes                             |
| ---------------------------- | --------------------------------- |
| File system                  | Max 100 MB, temporary (ephemeral) |
| Azure Blob                   | Persistent storage                |
| Log Analytics / App Insights | Unified search, alerts, metrics   |

---

## 🧠 Important Concepts for AZ-204
...
| Tool                            | What It Does                                    |
| ------------------------------- | ----------------------------------------------- |
| **Kudu** (Advanced Tools)       | Access log stream, console, and files           |
| **Live Log Stream**             | Real-time output from `stdout`                  |
| **Application Insights**        | Full observability: metrics, traces, exceptions |
| **Diagnose and Solve Problems** | Web-based wizard to troubleshoot app failures   |
...
| ---------------------------------- | ----------------------------- |
| See app-level logs like exceptions | App Logging                   |
| Track all HTTP requests/responses  | Web Server Logs               |
| Get real-time logs (live tail)     | Log Stream                    |
| Troubleshoot 500 errors            | Detailed Error Messages       |
| Diagnose why a request fails       | Failed Request Tracing (FREB) |
| Persistent log storage             | Azure Blob                    |
| Centralize logs from many apps     | Application Insights          |

---

## 🔒 Logging for Container Apps

If running a container in App Service:

* Logs = captured from **`stdout` and `stderr`**
* View with `az webapp log tail`
